    ...
    def run_auto_game(self, max_moves=100):
        ...
        for move in range(1, max_moves + 1):
            ...
            if best_chain:
                ...
                best_pair = max(
                    candidate_pairs,
                    key=lambda ch: self.evaluate_chain_smart(ch),
                )

                self.last_move_type = "fallback_pair"
                self.last_move_direction = (
                    f"{best_pair[0][0]}_{best_pair[0][1]}_"
                    f"{best_pair[1][0]}_{best_pair[1][1]}"
                )

                if self.perform_chain_swipe_mt(best_pair, steps=1):
            ...
            if self.show_board_each_move:
                ...
                print_board(self.board, confidence_board)
