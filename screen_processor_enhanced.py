"""
Enhanced ScreenProcessor with improved error handling, caching,
and backup methods for the 2248 bot project
"""
import subprocess
import cv2
import numpy as np
...
class EnhancedScreenProcessor:
    def __init__(self, config_manager):
        ...
        self.image_cache = {}
        self.cache_size_limit = 10  # Limit cache size to prevent memory issues
    ...
    def adb_command(self, cmd, capture_output=False, retries=None):
        """Execute ADB command with retry mechanism."""
        if retries is None:
        ...
        for attempt in range(retries + 1):
            try:
                if capture_output:
                    ...
                    result = subprocess.run(cmd, shell=True, check=True, stdout=subprocess.DEVNULL, timeout=10)
                    return result.returncode == 0
            except subprocess.TimeoutExpired:
                print(f"⚠️ ADB command timeout (attempt {attempt + 1}/{retries + 1}): {cmd[:50]}...")
                if attempt < retries:
                    time.sleep(self.retry_delay)
                    continue
                return None
            except Exception as e:
                print(f"❌ ADB command failed (attempt {attempt + 1}/{retries + 1}): {e}")
                if attempt < retries:
                    time.sleep(self.retry_delay)
                    continue
                return None
    ...
    def grab_screen_cv2(self):
        ...
        try:
            ...
            print("❌ Скриншот не получен: таймаут")
            return None
        except subprocess.CalledProcessError:
            print("❌ Скриншот не получен: ошибка ADB")
            return None
    ...
    def cache_image(self, cache_key, img):
        """Cache an image with size management"""
        if len(self.image_cache) >= self.cache_size_limit:
            # Remove oldest entry (in insertion order)
